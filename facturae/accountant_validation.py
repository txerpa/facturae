...
class Validation:
    ...
    def validate_totals(self, invoice_elem):
        ...
        # outputs
        sum_taxes_taxes_outputs = self._sum_taxes_amount(
            invoice_elem.TaxesOutputs, TaxTypeCode.IVA
        )
        ...
        if hasattr(invoice_elem, "TaxesWithheld"):
            sum_taxes_taxes_withheld = self._sum_taxes_amount(
                invoice_elem.TaxesWithheld, TaxTypeCode.IRPF
            )
        ...
        if total_gross_amount != total_gross_amount_before_taxes:
            raise AccountantValidation(
                "TotalGrossAmount not equals to TotalGrossAmountBeforeTaxes"
            )
